<script lang="ts">
	...
	let { title, turns, currentIndex, trackType, handleTurnSelect, height }: Props = $props();
	...
	const getSegmentColor = (turn: Turn, index: number) => {
		...
		return hasData ? 'bg-neutral-400 dark:bg-neutral-500' : 'bg-neutral-200 dark:bg-neutral-700';
	};

	const getTurnTooltip = (turn: Turn) => {
		const turnNum = turn.index + 1;
		if (trackType === 'screenshots') {
			return turn.screenshot ? `Turn ${turnNum}: Screenshot` : `Turn ${turnNum}: No screenshot`;
		} else {
...
<div class="relative" style="height: {height}">
	<div
		class="absolute top-0 left-0 z-10 bg-neutral-100 px-1 text-xs text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400"
	>
		{title}
	</div>
	...
	<div class="flex h-full items-center pt-4">
		{#each turns as turn, index}
			<div
				class="relative h-16 cursor-pointer border-r border-neutral-300 transition-colors hover:opacity-80 dark:border-neutral-600 {getSegmentColor(
					turn,
					index
			...
			>
				<div class="absolute top-1 left-1 text-xs font-medium text-white opacity-90">
					{turn.index + 1}
				</div>

				<div class="absolute top-1 right-1 flex gap-1">
					{#if turn.screenshot}
						<svg class="h-3 w-3 text-white opacity-80" fill="currentColor" viewBox="0 0 20 20">
							<path
								fill-rule="evenodd"
				...
				{#if turn.screenshot}
					<!-- Left side: Turn info -->
					<div class="absolute top-4 bottom-1 left-1 flex w-16 flex-col justify-between">
						{#if turn.thought}
							<div class="text-xs leading-tight text-white opacity-90">
					...
					<div class="absolute top-1 right-1 bottom-1 w-80 overflow-hidden rounded bg-black/20">
						<img
							src={turn.screenshot}
				...
				{:else}
					...
					{#if turn.thought}
						<div
							class="absolute right-1 bottom-1 left-1 text-xs leading-tight text-white opacity-90"
						>
