<script lang="ts">
  ...
  let { screenDimensions }: Props = $props();
...
<div
  class="cursor"
  style="
    left: {((position[0] || screenDimensions.width / 2) /
    screenDimensions.width) *
    100}%;
    top: {((position[1] || screenDimensions.height / 2) /
...
{#if isDragging}
  <svg class="drag-line">
    <line
      x1="{(dragStart[0] / screenDimensions.width) * 100}%"
      y1="{(dragStart[1] / screenDimensions.height) * 100}%"
      x2="{(((dragEnd[0] - dragStart[0]) * dragProgress + dragStart[0]) /
        screenDimensions.width) *
        100}%"
      y2="{(((dragEnd[1] - dragStart[1]) * dragProgress + dragStart[1]) /
        screenDimensions.height) *
        100}%"
